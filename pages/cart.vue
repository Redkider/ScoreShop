...
<script>
	...
	import {cartItemClear,cartItemDel,cartItemList,cartItemUpdateNum,cartItemUpdateSku,productDetail} from "@/api/product";
	export default {
		onShow() {
		...
		data() {
			return {
				...
				isStop: false,
				cartList: [
		{"id":"1","user":"15021133045","productName":"【低聚木糖】家庭装（5+1）盒","productId":"65","amount":1,"price":"3894","date":"2019/08/01 11:29:02","state":"未付款"},
		{"id":"2","user":"15021133045","productName":"【低聚木糖】特惠装（5+1）盒","productId":"63","amount":1,"price":"1298","date":"2019/08/01 11:29:02","state":"未付款"},
		{"id":"3","user":"15021133045","productName":"【低聚木糖】精品装（5+1）盒","productId":"64","amount":1,"price":"2596","date":"2019/08/01 11:29:02","state":"未付款"},
		{"id":"4","user":"15021133045","productName":"【低聚木糖】尊贵装（5+1）盒","productId":"66","amount":1,"price":"6490","date":"2019/08/01 11:29:02","state":"未付款"}
	],
				userInfo:{},
				token: true,
				oldDiscount: 0,
		...
		methods: {
			...
			async getProductDetail(row) {
				...
				await this.$get('http://localhost:8080/static/api/productDetail.json', {
					id: row.product_id,
				}).then(async r => {
			...
			async getCartItemList(type) {
				//await this.$get(`${cartItemList}`, {}, {}, this).then(r => {
				 await this.$get('http://localhost:8080/static/api/cartItemList.json').then(r => {
				  console.log(type)
				  this.loading = false;
					if (type === 'refresh') {
