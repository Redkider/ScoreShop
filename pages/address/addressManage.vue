<template>
	<view class="content" >
		...
		<view class="row b-b">
			...
			<input class="input" type="number" v-model="addressData.phone"  @blur="handleMobileChange" placeholder="收货人手机号码" placeholder-class="placeholder" />
		</view>
		<view class="row b-b">
		...
		<view class="row default-row">
			...
			<switch :checked="addressData.is_default ? true : false"  @change="switchChange" />
		</view>
...
<script>
	...
	import address from '@/components/address/address.nvue';
	...
	export default {
	    ...
	    // 获取收货地址
			async getAddressDetail(id) {
				await this.$get('http://localhost:8080/static/api/addressList.json', {
					id
				}).then(async r => {
			    this.loading = false;
				for(var i=0;i<r.data.length;i++){
					if(r.data[i].id==id){
						this.addressData = r.data[i];
						return;
					}
				}
			...
			async getProvinceList() {
				...
				let city_name = null;
				let area_name = null;
				await this.$get('http://localhost:8080/static/api/address.json').then(async r => {
            this.loading = false;
						...
						await this.$get('http://localhost:8080/static/api/address.json', {
							pid: province_id
						}).then(async r => {
