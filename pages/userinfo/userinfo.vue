		...
		<view class="input-content">
			<form @submit="toUpdateInfo">
				...
				<view class="input-item">
					...
					<input type="text" 
					:value="profileInfo.unionpayname" 
					name="unionpayname" 
					disabled
					@tap="toAdministrator"
				...
				<view class="input-item">
					...
					<input type="text" 
					@tap="toAdministrator"
					:value="profileInfo.unionpayaddress" 
					name="unionpayaddress"
					 disabled
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 获取用户信息
			async getMemberInfo () {
				await this.$get('http://localhost:8080/static/api/memberInfo.json').then(r => {
			    this.loading = false;
