<template>
  <view class="index">
    ...
    <rf-floor-index
        :list="hotProductList"
        :header="{title: '兑换专区', desc: 'Exchange Zone'}"
        @toBanner="indexTopToDetailPage"
    ...
    <!--猜你喜欢-->
    <rf-floor-index
        :list="RepeatPurchaseList"
        :header="{title: '复购专区', desc: 'Repeat Purchase'}"
	...
	export default {
		...
		methods: {
			...
			//await this.$get(`${indexList}`, {}).then(async r => {
				await this.$get('http://localhost:8080/static/api/indexList.json', {}).then(async r => {
					this.loading = false;
					// 获取商户列表
					...
					this.RepeatPurchaseList = r.data.Repeat_Purchase;
			...
			//	await this.$get(`${merchantIndex}`, {}).then(r => {
				await this.$get('http://localhost:8080/static/api/indexList.json', {}).then(r => {
