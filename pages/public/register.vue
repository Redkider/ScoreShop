		...
		<view class="wrapper">
			...
			<view class="input-content">
				<form @submit="toRegister" >
					<view class="input-item" v-show='false'>
						<input  type="number" name="superior" :value="superior" />
					</view>
...
<!--			</view>-->
		...
		<view class="footer">
			注册表示同意
			<text class="protocol" @tap="navTo(`/pages/about/detail?field=protocol_register&title=注册协议`)">低聚木糖积分商城使用协议 / 注册协议</text>
		</view>
	</view>
</template>
...
<script>
	...
	export default{
		...
		onLoad(){
               this.superior=this.$route.query.id  ||0;
		},
		methods: {
			...mapMutations(['login']),
			blurMobileChange(e) {
			...
			getSmsCode () {
			    ...
			    this.$api.msg("验证码发送成功, 验证码是: 123456") ;   //${r.data}`);
			...
			async toRegister(e){
				...
				const rule = [
					...
					{name:"password", checkType : "string", checkRule:"6,6",  errorMsg:"密码长度为6位"},
					{name:"password_repetition", checkType : "notnull", checkRule:"",  errorMsg:"请输入确认密码"},
					{name:"realname", checkType : "string", checkRule:"2,8",  errorMsg:"姓名应为2-8字符"}
				];
