		...
		methods: {
          ...
          rule[1] = {name:"code", checkType : "notnull", checkRule:"",  errorMsg:"请输入验证码"};
			...
			handleLoginByPass (params) {
				...
				//this.$post('http://localhost:8080/static/api/mobileLogin.json', {
				this.$get('http://localhost:8080/static/api/mobileLogin.json', {
					...params
				}).then(r=>{
					
					for(var i=0;i<r.data.length;i++){
						if(r.data[i].phone==params.mobile&&r.data[i].password==params.password){
							this.$api.msg('恭喜您，登录成功！');
							 this.userInfo=r.data[i];
