            ...
            <text v-else>暂无限购说明</text>
				...
				<view class="c-row b-b">
					...
					<view class="con-list" v-if="productDetail.point_exchange_type">
						...
						<text v-if="productDetail.point_exchange_type== 3">最大可使用积分: {{ productDetail.max_use_point }} </text>
  ...
  import moment from '@/utils/moment';
	...
	export default{
        ...
        if (html) {
			...
			givePointFilter(val) {
				return parseInt(val.integral_give_type, 10) == 0 ?
						parseInt(val.give_point, 10)  :
						parseInt(val.price*val.percentum/100, 10);
			}
		},
		data() {
			return {
      ...
      return {
		...
		methods:{
			...mapMutations(['handle','addCartList','getcartNum']),
			//服务弹窗
			showService() {
          ...
          uni.showModal({
			...
			//	await this.$post(`${couponReceive}`, {
				await this.$get('http://localhost:8080/static/api/couponList.json', {
					//id: item.id
				}).then((r) => {
			...
			async getProductDetail (id) {
			   //await this.$get(`${productDetail}`, {
				await this.$get('http://localhost:8080/static/api/productDetail.json?id=`${id}`', {
					id,
				}).then(async r => {
					this.loading = false;
		 console.log(r.data)
		  for(var i=0;i<r.data.length;i++){
			  if(r.data[i].id==id){
				   return this.productDetail = r.data[i];
        ...
        /*  this.specSelected = [];
          ...
          /* if (this.specSelected.length < this.productDetail.base_attribute_format.length){
			...
			async handleCartItemCreate () {
				...
				await this.$get('http://localhost:8080/static/api/cartItemList.json', {
					...
					var cartNum=0
					for(var i=0;i<r.data.length;i++){
						if(r.data[i].id==sku_id){
							r.data[i].amount+=num;
						}else{
							
						}
						cartNum+=r.data[i].amount
					}
					uni.setStorageSync('cartNum', cartNum);
					this.cartNum = cartNum; 
					this.getcartNum(this.cartNum);
					this.$api.msg('添加成功，在购物车等');
					this.addCartList(r.data)
					}).catch(err => {
						console.log(err)
					})
          ...
          this.favorite ? this.handleCollectDel() : this.handleCollectCreate();
			...
			//	await this.$post(`${collectCreate}`, {
					...
					for(var i=0;i<r.data.length;i++){
						if(r.data[i].productId==this.product_id&&r.data[i].user==this.userInfo.phone){
							
							this.favorite=!r.data[i].favorite
							console.log("收藏:"+this.favorite)
							this.handle(r.data)
							uni.setStorage({
							    key: 'handleCollect',
							    data: r.data
			...
			async handleCollectDel() {
				...
				await this.$get('http://localhost:8080/static/api/handleCollect.json').then(r => {
					for(var i=0;i<r.data.length;i++){
						if(r.data[i].productId==this.product_id&&r.data[i].user==this.userInfo.phone){
							r.data[i].favorite=true
							this.favorite=!r.data[i].favorite
							console.log("取消:"+this.favorite)
						}
					}
					//this.favorite = !this.favorite;
					this.$api.msg('取消收藏成功');
