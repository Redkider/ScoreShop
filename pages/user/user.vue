	...
	<view class="user">
		...
		<view class="user-section">
          ...
          @tap="navTo(userInfo ? '/pages/userinfo/userinfo' : 'login')">
				<view class="portrait-box">
					<image class="portrait" :src="(userInfo && userInfo.head_portrait	) || userInfo.headimgurl || '/static/missing-face.png'"></image>
					<text class="username">
		...
		>
			...
			<view class="history-section icon">
				...
				<view v-if="token">
					<rfTab :tab-list="tabList" :tabCur="TabCur" @change="tabChange" tab-class="text-center text-black bg-white" select-class="text-blue text-xl">
					</rfTab>
					<view v-show="true">
						<scroll-view scroll-x class="h-list" v-if="tableData.length>0">
							<!-- <view class="h-item" v-for="item in teamClass1List" :key="item.id">
								
								<image class="h-item-img" @tap.stop="navTo(`/pages/product/product?id=${item.product.id}`)"
											 :src="item.photo"
											 mode="aspectFill">
								</image>
								<text class="in1line">{{ item.name }}</text><text class="in1line">{{ item.name }}</text>
							</view> -->
						...
						<view class="no-foot-print" v-if="tableData.length === 0" @tap="navTo('/pages/product/list')">
...
<script>
	...
	import rfTab from '@/components/rf-tab/rf-tab';
	import uniTable from '@/components/uni-table/uni-table';
			...
			return {
				tableData:[],
				 columns:[
				        ...
				        { title:'消费积分',key:'score','width':150, },
				        { title:'推广人数',key:'popularize','width':150,},
				         ],
				tabList: [{ name: 'TeamClass1' }, { name: 'TeamClass2' }],
				TabCur: 0,
				tableData:[],
    ...
    async onShow() {
		...
		methods: {
			tabChange(index) {
			        this.TabCur = index;
					if(this.TabCur==0){
					this.tableData=this.userInfo.team.class1;
					console.log(this.tableData)
					
								
					 }else if(this.TabCur==1){
					this.tableData=this.userInfo.team.class2;	 	 
			...
			async initData() {
				...
				console.log("余额:"+this.userInfo.remainingSum)
			 ...
			 // this.token = uni.getStorageSync('accessToken') || undefined;
		      ...
		      if (uni.getStorageSync('cartNum')) {
			    ...
			    this.loading = false;
			    uni.removeTabBarBadge({index: 2})
				this.amountList[0].value = this.userInfo.remainingSum || 0;
				this.amountList[1].value = this.userInfo.exchangeScore || 0;
				this.amountList[2].value = this.userInfo.redEnvelope || 0;
			}
			},
			// 获取用户信息
      async getMemberInfo() {
		//this.$get(memberInfo).then(r => {
		 ...
		 console.log(r.data)
		  this.loading = false;
          this.amountList[0].value = r.data.remainingSum || 0;
          this.amountList[1].value = r.data.exchangeScore || 0;
          this.amountList[2].value = r.data.redEnvelope || 0;
